...
class BelongsToOrder:
    @declared_attr
    def exchange_id(cls):
        return Column(Integer, ForeignKey(Exchange.id), nullable=False)
    ...
    def __table_args__(cls):
        idx_name = f"{cls.__name__}-exchange-id-exchange-order-id-idx"  # type: ignore
        return (
            ForeignKeyConstraint(
                ["exchange_id", "exchange_order_id"],
                [Order.exchange_id, Order.exchange_order_id],
            ),
...
class Coin(Base):
    __tablename__ = TABLE_PREFIX + "coins"
...
class Exchange(Base):
    __tablename__ = TABLE_PREFIX + "exchanges"
    name = Column(String)
    id = Column(Integer, primary_key=True)
    markets = relationship("ExchangeMarket", back_populates="exchange")
    markets_with_data = relationship(
        "ExchangeMarket",
    ...
    def to_dict(self, include_markets=False):
        result: Dict[str, Any] = dict(id=self.id, name=self.name)
        if include_markets:
...
class Order(Base):
    __tablename__ = TABLE_PREFIX + "orders"
    ...
    sell_sym_id = Column(String, ForeignKey(Coin.symbol), nullable=False, index=True)
...
class MarketOrderFunds(BelongsToOrder, Base):
    __tablename__ = TABLE_PREFIX + "market_order_funds"
...
class OrderSize(BelongsToOrder, Base):
    __tablename__ = TABLE_PREFIX + "order_sizes"
...
class Market(Base):
    __tablename__ = TABLE_PREFIX + "markets"
    first_coin_id = Column(
        String, ForeignKey(Coin.symbol), nullable=False, index=True, primary_key=True
    )
    first_coin = relationship("Coin", foreign_keys=[first_coin_id])
    second_coin_id = Column(
        String, ForeignKey(Coin.symbol), nullable=False, index=True, primary_key=True
    )
    second_coin = relationship("Coin", foreign_keys=[second_coin_id])

    def __eq__(self, other):
        return (self.first_coin_id, self.second_coin_id) == (
            other.first_coin_id,
            other.second_coin_id,
        )
...
class ExchangeMarket(Base):
    __tablename__ = TABLE_PREFIX + "exchange_markets"
    ...
    quoted_volume_id = Column(String, ForeignKey(Coin.symbol), nullable=False)
    original_name = Column(String, nullable=False)

    agg_orders_count = Column(Integer, nullable=False, server_default="0")

    first_coin_id = Column(String, ForeignKey(Coin.symbol), nullable=False, index=True)
    second_coin_id = Column(String, ForeignKey(Coin.symbol), nullable=False, index=True)
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ...
            [Market.first_coin_id, Market.second_coin_id],
        ),
    ...
    def __repr__(self):
        return "ExchangeMarket(name='{0}', exchange_id={1})".format(
            self.name, self.exchange_id
        )


class Trade(Base):
    __tablename__ = TABLE_PREFIX + "trades"

    exchange_trade_id = Column(String, primary_key=True)
    exchange_id = Column(
        Integer, ForeignKey(Exchange.id), nullable=False, primary_key=True
    )
...
class AggOrder(Base):
    __tablename__ = TABLE_PREFIX + "aggregate_orders"
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.first_coin_id > self.second_coin_id:
            self.first_coin_id, self.second_coin_id = (
                self.second_coin_id,
                self.first_coin_id,
            )

    def pk_hash(self):
        pk = f"{self.last_update_id}{self.exchange_id}{self.order_type}{self.price}".encode(
            "utf-8"
        )
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(
            ["first_coin_id", "second_coin_id", "exchange_id"],
            [
                ExchangeMarket.first_coin_id,
                ExchangeMarket.second_coin_id,
                ExchangeMarket.exchange_id,
            ],
        ),
    )
...
class OrderBookSnapshot(Base):
    __tablename__ = TABLE_PREFIX + "order_book_snapshots"
...
class Event(Base):
    __tablename__ = TABLE_PREFIX + "events"
