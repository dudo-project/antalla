...
class CoinbaseListener(WebsocketListener):
    def __init__(
        ...
        super().__init__(
            exchange, on_event, markets, ws_url, session=session, event_type=event_type
        )
        self._all_symbols = []
    ...
    def _get_last_update_ids(self):
        ...
        market_update_ids = self.session.execute(
            ...
            inner join {models.Exchange.__tablename__} ex on ag.exchange_id = ex.id where ex.name = 'coinbase'
            group by buy_sym_id, sell_sym_id, ex.name;
            """
        )
        for market in list(market_update_ids):
            last_update_id = (
                market["max_update_id"] if market["max_update_id"] is not None else 0
            )
            key = (
                str(market["exchange"])
                + str(market["buy_sym_id"])
                + str(market["sell_sym_id"])
            )
    ...
    def _parse_snapshot(self, snapshot):
        ...
        market_key = (
            self.exchange.name.lower() + buy_sym_id.upper() + sell_sym_id.upper()
        )
        ...
        logging.debug(
            " {} - aggregated order book snapshot - agg orders: {}".format(
                self.exchange.name, len(agg_orders)
            )
        )
    ...
    def _create_agg_orders(self, order_type, order_info, orders):
        parsed_orders = []
        market_key = (
            self.exchange.name.lower()
            + order_info["buy_sym_id"].upper()
            + order_info["sell_sym_id"].upper()
        )
        for order in orders:
    ...
    def _get_markets_uri(self):
        return settings.COINBASE_API + "/" + settings.COINBASE_API_PRODUCTS
    ...
    async def get_markets(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            logging.debug(
                "markets retrieved from %s: %s", self.exchange.name, incomplete_markets
            )
    ...
    async def _setup_connection(self, websocket):
        ...
        await self._send_message(
            websocket, "subscribe", self._all_markets, self._get_events()
        )
