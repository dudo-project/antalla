...
class BinanceListener(WebsocketListener):
    ...
    def _get_ws_url(self):
        ...
        for stream in self._get_events():
            for pair in self.markets:
                streams.append("".join(pair.lower().split("_")) + "@" + stream)
        self._ws_url = settings.BINANCE_COMBINED_STREAM + "/".join(streams)
    ...
    def _get_event_data_collected(self, data_type):
        if data_type == "trade":
            ...
            logging.debug(
                "unknown event type for 'data collected' - {}".format(data_type)
            )
            return "Unknown"
            ...
            if "depth" in events:
                ...
                for pair in self.markets:
                    ...
                    uri = (
                        ...
                        + "/api/v1/depth?symbol="
                        + "".join(pair.upper().split("_"))
                        + "&limit="
                        + str(DEPTH_SNAPSHOT_LIMIT)
                    )
    ...
    def _parse_snapshot(self, snapshot, pair):
        ...
        logging.debug(
            "parsed %d orders in depth snapshot for pair '%s'",
            len(orders),
            pair.lower(),
        )
    ...
    def _parse_depthUpdate(self, update):
        ...
        logging.debug(
            "parsed %d orders in 'depth update' for pair '%s'",
            len(orders),
            "".join(pair),
        )
        return self._parse_agg_orders(orders)
    ...
    async def fetch_all_symbols(self, session):
        exchange_info = await self._fetch(
            session, self._get_uri(settings.BINANCE_API_INFO)
        )
        all_symbols = set()
    ...
    async def get_markets(self):
        async with aiohttp.ClientSession() as session:
            ...
            markets = await self._fetch(
                session, self._get_uri(settings.BINANCE_API_MARKETS)
            )
        ...
        for market in markets:
            ...
            if len(pair) == 2:
                ...
                new_market = models.Market(
                    first_coin_id=pair[0], second_coin_id=pair[1]
                )
                ...
                logging.debug(
                    "parse markets for '{}' - invalid market format: '{}' is not a pair of markets - IGNORE".format(
                        self.exchange.name, market
                    )
                )
        return [
