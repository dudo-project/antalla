...
def upgrade():
    op.create_table(
        ...
        sa.Index(
            "market_orders_index", "first_coin_id", "second_coin_id", "exchange_id"
        ),
    )
    op.execute(
        f"""
    ...
    RETURNS TRIGGER AS $$
    BEGIN
        UPDATE {TABLE_PREFIX}exchange_markets e
    ...
    AFTER INSERT ON {TABLE_PREFIX}aggregate_orders
    FOR EACH ROW
...
def downgrade():
    op.drop_table(TABLE_PREFIX + "aggregate_orders")
