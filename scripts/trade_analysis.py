...
def get_exchange_trades(buy_sym_id, sell_sym_id, exchange):
    query = (
        "select exchange_trade_id, name, timestamp, trade_type, buy_sym_id, sell_sym_id, price,"
...
def plot_hourly_trade_vol(buy_sym_id, sell_sym_id, exchanges):
    ...
    ax.set(
        xlabel="Timestamp",
        ylabel="Trade volume (" + buy_sym_id + ")",
...
def is_original_market(buy_sym_id, sell_sym_id, exchange):
    query = f"""select name, first_coin_id, second_coin_id, original_name
                from {models.ExchangeMarket.__tablename__} em
                inner join {models.Exchange.__tablename__} e on e.id = em.exchange_id
                where name = '{exchange.lower()}'"""
...
def invert_trades(raw_trades):
    ...
    for trade in list(raw_trades):
        ...
        modified_trade["size"] = (
            float((trade[6] * trade[7]) * (float(1.0 / trade[6]))) * trade[6]
        )
        modified_trade["volume"] = float(
            (trade[6] * trade[7]) * (float(1.0 / trade[6]))
        )
...
def plot_volume_bar_chart(buy_sym_id, sell_sym_id, exchange, plot_id):
    ...
    p1 = axs[0].bar(
        df_bins.index, df_bins["sell_volume"], width=0.01, color="steelblue"
    )
    ...
    axs[0].set(
        title=exchange
        + ": Traded Volume and Price per Hour: "
        + buy_sym_id
        + "-"
        + sell_sym_id
    )
