...
def get_markets(exchange, symbol): 
    markets = db.session.execute("select original_name from exchange_markets em inner join exchanges e on em.exchange_id = e.id where \
        e.name=:exchange and (first_coin_id=:symbol or second_coin_id=:symbol)", {"symbol":symbol.upper(), "exchange":exchange.lower()}) 
